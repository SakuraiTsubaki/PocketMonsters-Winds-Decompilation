...
This document defines shared repository conventions for this planned decompilation project. Do not assume a platform, executable format, engine, build, region, revision, or resource layout until it has been authoritatively identified and verified.

## Naming conventions

- Prefer stable, descriptive, ASCII-safe file and directory names unless a verified target format requires otherwise.
...
- Include version, language, revision, or region identifiers in paths or metadata only after those targets are verified.
...
- Byte-identical assets shared by verified versions, revisions, or languages should normally be stored once and referenced through metadata, manifests, or version maps instead of duplicated.
...
- Record meaningful differences only after the relevant targets have been identified.
...
- repository path
- verified target version / region / language / revision
...
Do not invent unknown metadata. Use an explicit `unknown`, `TBD`, or equivalent state until information is verified.

## Evidence and provenance

- Separate confirmed observations from hypotheses.
- Do not make target-specific technical claims before the target is identified.
...
Do not copy platform-specific conventions from an earlier generation. Once the actual executable, archive, resource, and build layouts are authoritatively verified, add naming, directory, manifest, and verification rules that reflect that target architecture.